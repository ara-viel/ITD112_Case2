...
import EduCSV from "../csv/EduCSV";
import { getEduc, addEduc, updateEduc, deleteEduc, deleteAllEduc } from "../services/educationService";

Chart.register(ChartDataLabels);

const EducationDataset = () => {
  const [edu, setEduc] = useState([]);
  const [form, setForm] = useState({
    eduAttainment: "",
    ...Object.fromEntries(Array.from({ length: 40 }, (_, i) => [1988 + i, ""])),
    notReported: "",
  });
  ...
  const fetchData = async () => {
    const data = await getEduc();
    const cleaned = data.map((d) => ({
      id: d.id,
      eduAttainment: d.eduAttainment || "",
      ...Object.fromEntries(
  ...
  const handleChange = (e) =>
    setForm({ ...form, [e.targeret.name]: e.target.value });

  const handleAdd = async () => {
    if (!form.eduAttainment.trim()) {
      alert("⚠️ Please enter an educational attainment.");
      return;
    }
  ...
  const handleSaveEdit = async () => {
    await updateEduc(editId, editForm);
    setEditId(null);
    fetchData();
  };
  ...
  const filteredData = edu.filter((item) =>
    item.eduAttainment.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const colors = [
    ...
    "#6366f1", "#f43f5e", "#a855f7", "#22c55e", "#eab308"
  ];

  const lineData = {
    labels: years,
    datasets: filteredData.map((a, i) => ({
      label: a.eduAttainment,
  ...
  const lineOptions = {
    ...
    plugins: {
      ...
      title: { 
        display: true, 
        text: "Educational Attainment Trends (1988–2020)",
  ...
  const barOptions = {
    ...
    plugins: {
      ...
      title: { 
        display: true, 
        text: "Educational Attainment Composition (Stacked)",
        font: { size: 18, weight: "600", family: "'Inter', sans-serif" },
        padding: { bottom: 20 }
      },
      datalabels: { display: false },
    ...
    <div style={{ padding: "0", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ 
        ...
        <p style={{ 
          ...
          Emigration data from 1988 to 2020
        </p>
      </div>
      ...
      <div style={{ 
        ...
        <EduCSV collectionName="education" />
      </div>
      ...
      <div style={{ 
        ...
        <input
          type="text"
          placeholder="🔍 Search by educational attainment..."
          value={searchQuery}
          ...
          <div style={{
            ...
            <table style={{ 
              ...
              <thead style={{ 
                ...
                <tr>
                  <th style={tableHeaderStyle}>Educational Attainment</th>
                  {years.map((y) => (
              ...
              <tbody>
                {filteredData.map((e, idx) => (
                  <tr key={e.id} style={{
                    ...
                    <td style={tableCellStyle}>{e.eduAttainment}</td>
                    {years.map((y) => (
...
export default EducationDataset;