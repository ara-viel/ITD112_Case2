...
import {
  ...
  deleteAllCountry,
  deleteAllCountries,
} from "../services/allCountriesService";
import AllCountriesCSV from "../csv/AllCountriesCSV";
...
const AllCountriesDataset = () => {
  ...
  useEffect(() => {
    fetch("/src/data/worldCountries.json")
      .then((res) => res.json())
  ...
  const handleSaveEdit = async () => {
    await updateAllCountry(editForm.id, editForm);
    setEditForm(null);
    fetchData();
  };
  ...
  const chartData = useMemo(() => {
    return countries.map((c) => {
      const total =
        selectedYear === "All"
          ? Object.keys(c)
              .filter((k) => !["id", "country"].includes(k))
              .reduce((sum, y) => sum + Number(c[y] || 0), 0)
          : Number(c[selectedYear] || 0);
      return { country: c.country, value: total };
    });
  ...
  useEffect(() => {
    let chartInstance;
    if (barChartRef.current && top5Data.length > 0) {
      chartInstance = new Chart(barChartRef.current, {
        type: "bar",
        data: {
          labels: top5Data.map(d => d.country),
          datasets: [{
            label: "Number of Emigrants",
            data: top5Data.map(d => d.value),
        ...
        options: {
          ...
          plugins: {
            ...
            title: {
              display: true,
              text: `Top 5 Destination Countries (${selectedYear === "All" ? "All Years" : selectedYear})`,
    ...
    <div style={{ padding: "0", fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ 
        ...
        <p style={{ 
          ...
          Country-by-country emigration data from 1988 to 2020
        </p>
      </div>
      ...
      <div style={{
        ...
        <AllCountriesCSV collectionName="destination" />
                ...
                <Geographies geography={geoData}>
                  {({ geographies }) =>
                    geographies.map((geo) => {
                      ...
                      const countryData = chartData.find(
                        (c) => cleanName(c.country) === cleanName(countryName)
                      );
                      const color = countryData ? colorScale(countryData.value) : "#e5e7eb";
                      return (
                        <Geography
                          key={geo.rsmKey}
                          geography={geo}
...
export default AllCountriesDataset;